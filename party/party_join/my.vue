		...
		<div v-else class="sglist">
			...
			<div v-for="(item,index) in list" :key="index" class="sglist-item">
				...
				<div class="flex">
					...
					<div class="btn-mini btn-outline-danger mgr-5" @click="cancel(item)" url="/module.php?m=party_join&a=delete&ajax=1&pid={$c.id}">取消参与</div>
	...
	export default{
		...
		methods:{
			...
			getPage:function(){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/module.php?m=party_join&a=my&ajax=1",
					data:{
						type:this.type
					},
					dataType:"json",
			...
			cancel:function(item){
				...
				uni.showModal({
					...
					success:function(e){
						if(e.confirm){
							that.app.get({
								url:that.app.apiHost+"/module.php?m=party_join&a=delete&ajax=1",
								data:{
									pid:item.id
								},
								success:function(res){
