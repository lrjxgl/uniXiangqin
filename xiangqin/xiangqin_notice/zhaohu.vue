...
export default{
	...
	created:function(){
		var s=uni.getStorageSync("xiangqin-notice-zhaohu"); 
		if(s!=null){
			this.type=s;
		} 
		this.getPage();
	...
	methods:{
		setType:function(type){
			this.type=type;
			uni.setStorageSync("xiangqin-notice-zhaohu",type); 
			this.per_page=0;
			this.isFirst=true;
			this.getList();
		},
		getPage:function(){
			var that=this;
			that.app.get({
				url:that.app.apiHost+"/module.php?m=xiangqin_zhaohu&a=my&ajax=1",
				data:{
					type:this.type
				},
		...
		pass:function(item){
			var that=this;
			that.app.get({
				url:that.app.apiHost+"/module.php?m=xiangqin_zhaohu&a=pass&ajax=1",
				dataType:"json",
				data:{
					id:item.id
				},
